// PureTidings Desktop - Standalone Native RSS & Content Reader
    ...
    feed_tree: [
        {
            ...
            type: 'folder',
            children: [
                { id: 'feed_verge', name: 'The Verge', type: 'feed', url: 'https://www.theverge.com/rss/index.xml', fetchOgImage: true },
                { id: 'feed_arstechnica', name: 'Ars Technica', type: 'feed', url: 'https://feeds.arstechnica.com/arstechnica/index', fetchOgImage: true },
                { id: 'feed_mkbhd', name: 'MKBHD (YouTube)', type: 'feed', url: 'https://www.youtube.com/feeds/videos.xml?channel_id=UCBJycsmduPZe6541G3gk22Q', fetchOgImage: true }
            ]
        },
        {
            id: 'folder_news',
            ...
            type: 'folder',
            children: [
                { id: 'feed_bbc', name: 'BBC News', type: 'feed', url: 'https://feeds.bbci.co.uk/news/rss.xml', fetchOgImage: true }
            ]
        }
    ],
    favorited_links: [],
...
function loadUserDataFromStorage() {
    try {
        const raw = localStorage.getItem('puretidings_userData');
        if (raw) {
            const parsed = JSON.parse(raw);
            return {
                feed_tree: parsed.feed_tree || DEFAULT_USER_DATA.feed_tree,
                favorited_links: sanitizeLinksArray(parsed.favorited_links),
                summary_links: sanitizeLinksArray(parsed.summary_links),
                read_links: sanitizeLinksArray(parsed.read_links),
                duration_cache: parsed.duration_cache || {}
            };
        }
    } catch(e) {
        console.error("Failed to parse local userData:", e);
    }
...
function saveUserDataLocally() {
    try {
        localStorage.setItem('puretidings_userData', JSON.stringify(userData));
    } catch (e) {
...
async function init() {
    console.log("🚀 PureTidings Desktop Init...");
    ...
    if (exportOpmlBtn) {
        exportOpmlBtn.onclick = () => {
            ...
            function walk(nodes) {
                nodes.forEach(n => {
                    if (n.type === 'folder') {
                        opml += `  <outline text="${escapeHTML(n.name)}">\n`;
                        walk(n.children || []);
                        opml += '  </outline>\n';
                    } else if (n.type === 'feed') {
                        opml += `  <outline type="rss" text="${escapeHTML(n.name)}" xmlUrl="${escapeHTML(n.url)}"/>\n`;
                    }
                });
            }
            ...
            a.download = `puretidings-feeds-${new Date().toISOString().slice(0,10)}.opml`;
            a.click();
        };
    }

    if (importOpmlBtn && importOpmlInput) {
        importOpmlBtn.onclick = () => importOpmlInput.click();
        importOpmlInput.onchange = (e) => {
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    if (outlines.length === 0) {
                        alert("No feeds found in the selected OPML file.");
                        return;
                    }
                    const newFeeds = [];
                    outlines.forEach(o => {
                    ...
                    if (!userData.feed_tree) userData.feed_tree = [];
                    userData.feed_tree.push(...newFeeds);
                    saveUserDataLocally();
                    renderSidebar(userData.feed_tree);
                    calculateAllUnreadCounts();
                    alert(`Successfully imported ${newFeeds.length} feeds!`);
                } catch (err) {
                    alert("Failed to parse OPML: " + err.message);
                }
            };
        ...
        backupJsonBtn.onclick = () => {
            const backup = {
                ...
                settings: {
                    ...
                    geminiAiPrompt: localStorage.getItem('gemini_ai_prompt') || '',
                    geminiYtPrompt: localStorage.getItem('gemini_yt_prompt') || '',
            ...
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = `puretidings-backup-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
            a.click();
        };
    }

    if (restoreJsonBtn && importJsonInput) {
        restoreJsonBtn.onclick = () => importJsonInput.click();
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    if (data.userData) {
                        ...
                        saveUserDataLocally();
                    } else if (data.local && data.local.feedTree) {
                        userData.feed_tree = data.local.feedTree;
                        if (data.local.readLinks) userData.read_links = data.local.readLinks;
                        if (data.local.favoritedLinks) userData.favorited_links = data.local.favoritedLinks;
                        if (data.local.summaryLinks) userData.summary_links = data.local.summaryLinks;
                        saveUserDataLocally();
                    }
                    if (data.rules) saveWebRules(data.rules);
                    else if (data.sync && data.sync.rules) saveWebRules(data.sync.rules);

                    if (data.settings) {
                        if (data.settings.geminiApiKey) localStorage.setItem('gemini_api_key', data.settings.geminiApiKey);
                        if (data.settings.geminiAiPrompt) localStorage.setItem('gemini_ai_prompt', data.settings.geminiAiPrompt);
                        if (data.settings.geminiYtPrompt) localStorage.setItem('gemini_yt_prompt', data.settings.geminiYtPrompt);
                    }
...
async function saveFeedTreeToDatabase() {
    try {
        ...
        console.error("Error saving feed tree locally:", e);
    }
}
...
async function updateCloudSettings(payload) {
    try {
        if (payload.favorited_links) {
            userData.favorited_links = sanitizeLinksArray(payload.favorited_links);
        }
        if (payload.summary_links) {
            userData.summary_links = sanitizeLinksArray(payload.summary_links);
        }
        if (payload.read_links) {
            userData.read_links = sanitizeLinksArray(payload.read_links);
        }
        if (payload.duration_cache) {
            userData.duration_cache = payload.duration_cache;
        }
        if (payload.rules) {
            localStorage.setItem('web_rules', JSON.stringify(payload.rules));
        }
        saveUserDataLocally();
    } catch (e) {
...
async function getYouTubeTranscript(url) {
    ...
    // 2. Native Desktop / InnerTube API fetch
    try {
        ...
        if (window.__TAURI__ && window.__TAURI__.core && window.__TAURI__.core.invoke) {
            const resText = await window.__TAURI__.core.invoke('post_url', {
                url: innerTubeUrl,
        ...
        if (!data || data.error) {
            return { status: 'error', message: `YouTube error: ${data?.error || 'Unknown error'}` };
        }
        ...
        let xmlText = '';
        if (window.__TAURI__ && window.__TAURI__.core && window.__TAURI__.core.invoke) {
            xmlText = await window.__TAURI__.core.invoke('fetch_url', { url: track.baseUrl });
        } else {
            const tResponse = await fetch(track.baseUrl);
            if (!tResponse.ok) return { status: 'error', message: 'Script exists on YouTube but could not be read.' };
            xmlText = await tResponse.text();
        }
...
async function fetchViaExtensionOrProxy(url) {
    // 1. In Tauri desktop app: use native Rust HTTP client (reqwest)
    if (window.__TAURI__ && window.__TAURI__.core && window.__TAURI__.core.invoke) {
        try {
            return await window.__TAURI__.core.invoke('fetch_url', { url });
        } catch (err) {
            console.error(`[Tauri Native Fetch Error] for ${url}:`, err);
            throw new Error(`Desktop fetch error: ${err}`);
        }
    }
