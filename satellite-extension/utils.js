...
function createPostElement(post, isRead, isFavorited, isKeywordMatch) {
  ...
  } else if (post.isEmail || (post.link && post.link.startsWith('imap:'))) {
  ...
  title.href = (post.isEmail || (post.link && post.link.startsWith('imap:'))) ? '#' : post.link;
  title.target = (post.isEmail || (post.link && post.link.startsWith('imap:'))) ? '_self' : '_blank';
